...
- **ESP-IDF 5.5.5 / C**：固件框架、驱动开发、组件管理和 CMake 构建
- **FreeRTOS**：使用任务、队列和互斥锁实现周期采集、后台配置及线程安全的数据快照
- **LVGL 9.5.0**：构建 1024 × 600 中文触控界面、导航页面、图表和数字输入控件
- **I2C 主机总线**：以 7 位地址 `0x3C` 访问 SW6306，完成寄存器读写、ADC 采集和参数配置
- **MIPI-DSI / GT911**：驱动 EK79007 显示屏并接收电容触摸输入
- **Waveshare ESP32-P4 BSP 3.0.1**：提供显示、触摸、背光和板级 I2C 等硬件适配
- **PSRAM / SPIFFS**：使用 200 MHz PSRAM XIP，并预留 SPIFFS 数据分区
- **UART 串口诊断**：提供状态读取、12 V 调试输出和关闭输出等诊断命令
